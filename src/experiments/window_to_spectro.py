...
class WindowToSpectro(BaseExperiment):
    ...
    def run(self):
        ...
        # time_stretch_t = torchaudio.transforms.TimeStretch(hop_length=512, n_freq=1025).to(run_device)
        # low_pass_t = torchaudio.functional.lowpass_biquad
        # high_pass_t = torchaudio.functional.highpass_biquad
        mel_t = torchaudio.transforms.MelScale(n_mels=129, sample_rate=44100).to(run_device)
        norm_t = torchaudio.transforms.ComplexNorm(power=2).to(run_device)
        ampToDb_t = torchaudio.transforms.AmplitudeToDB().to(run_device)
        ...
        samples = test_item["samples"].to(run_device)
        ...
        print(f"Spectrogram shape: {spectro.shape} -- {spectro}")
        librosa.display.specshow(spectro, sr=44100, hop_length=512,
        ...
        samples = full_samples.to(run_device)
        spectrogram = spectrogram_t(samples)
        # spectrogram = spectrogram.narrow(3, 0, 129)
